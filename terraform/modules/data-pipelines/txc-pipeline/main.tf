...
module "integrated_data_bods_txc_retriever_function" {
  ...
  function_name = "integrated-data-bods-txc-retriever-${var.environment}"
  zip_path      = "${path.module}/../../../../src/functions/dist/bods-txc-retriever.zip"
  role_arn      = aws_iam_role.integrated_data_bods_txc_retriever_role.arn
...
module "integrated_data_txc_retriever_function" {
  ...
  function_name      = "integrated-data-txc-retriever-${var.environment}"
  zip_path           = "${path.module}/../../../../src/functions/dist/txc-retriever.zip"
  role_arn           = aws_iam_role.integrated_data_txc_retriever_role.arn
  handler            = "index.handler"
  runtime            = "nodejs20.x"
  timeout            = 60
  memory             = 1024
  subnet_ids         = var.private_subnet_ids
  security_group_ids = [aws_security_group.integrated_data_txc_retriever_sg.id]
  schedule           = "cron(30 2 * * ? *)"
...
module "integrated_data_txc_unzipper_function" {
  source = "../../unzipper"
