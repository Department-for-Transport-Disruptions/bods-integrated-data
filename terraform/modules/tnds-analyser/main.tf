...
locals {
  tnds_observation_table_name = "integrated-data-tnds-observation-table-${var.environment}"
}
  ...
  permissions = [
    {
      ...
      Effect   = "Allow",
      Resource = "arn:aws:dynamodb:${var.aws_region}:${var.aws_account_id}:table/${local.tnds_observation_table_name}"
    }
  ]
