  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        "Resource" : [
          "arn:aws:states:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:execution:${aws_sfn_state_machine.integrated_data_lambda_trigger_sfn.name}/*"
        ]
      },
      {
        "Effect" : "Allow",
